...
app.get('/hospitalventilators',middleware.checkToken,function(req,res){
    console.log("Fetching details form ventilator collection");
    db.collection('hospitalventilators').find().toArray(function(err,result){
        if(err) console.log(err);
        res.json(result);
    })
});
...
app.post('/searchventilators',middleware.checkToken,function(req,res){
    ...
    db.collection('hospitalventilators').find(query).toArray().then(result=> res.json(result));
});
...
app.put('/updateventilatorsdetails',middleware.checkToken,function(req,res){
    ...
    db.collection('hospitalventilators').updateOne(vid,query2,function(err,result){
...
app.post('/addventilators',middleware.checkToken,function(req,res){
    ...
    db.collection('hospitalventilators').insertOne(query,function(err,result){
